...
- Root-cause pass, at the owner's instruction: every point the lead held against a reviewer gets a five-whys to its root cause (docs, a name, decision text, a rule message, or a real gap), and the root cause is removed; running as an architect analysis, fixes become cards.
- In debate, at the owner's instruction: reopening decision 17 (a domain service may consume across a boundary) and decision 15 (discriminated unions). The lead designer argues for, the architect against; the lead concludes and adopts a clear outcome. Codex has said no on runs 6 to 9, and after every reproduced defect was fixed its remaining objections are decisions it disputes (17 application-service mediation, 16 the kernel as a context, 15 unions, modules and value-object behaviour, 23 commands against events, 08 one file). A Codex yes needs one or more of those reopened; the lead defends them as written and will not run Codex again until told which, if any, to reopen.
