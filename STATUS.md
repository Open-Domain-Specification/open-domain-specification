...
- Debate concluded with both sides agreeing: decisions 17 and 18 stand, each with a testable reopening condition and its design pre-specified (decisions 17, 18, 21 amended 2026-09-10); card 117's blind model is the test for both. Codex has said no on runs 6 to 9, and after every reproduced defect was fixed its remaining objections are decisions it disputes (17 application-service mediation, 16 the kernel as a context, 15 unions, modules and value-object behaviour, 23 commands against events, 08 one file). A Codex yes needs one or more of those reopened; the lead defends them as written and will not run Codex again until told which, if any, to reopen.
