...
const RULES: CataloguedRule[] = [
	...
	},
	{
		...
		severities: ["error"],
		summary:
			"An entity is a kind of an entity of its own aggregate; a value object is a kind of one its own context declares, or one it borrows through a shared kernel, as a conformist, or as a customer-supplier downstream of the context that owns it.",
		why: "A kind is the same thing said more precisely, so it lives where the thing lives. An entity and the entity it is a kind of are loaded, saved and kept consistent through one root, and a parent in another aggregate would make one boundary's rules depend on another's. A value object is part of a context's ubiquitous language, and that language is legitimately shared in three places: a shared kernel, the declaration that two contexts keep part of one model between them; a conformist's relationship with its upstream, where the downstream has said it takes that model as it stands; and a customer-supplier downstream, where the negotiated interface's types are the supplier's published language and the customer has had a say in settling them.",
		fix: "Move the parent into the same aggregate as the kind, or the kind into the parent's; for a value object, declare the parent in this context, declare the shared kernel with the context that owns it if the two really do keep it in step, declare this context a conformist of that one if it takes that model as it stands, or declare that context a customer-supplier relationship with this context as the downstream if the two have negotiated the interface between them. Where the parent is an entity two contexts jointly own, the route is not a shared kernel between them: give the entity to a kernel context both consume, and hold the kind there too, since an entity has one home (decision 16).",
