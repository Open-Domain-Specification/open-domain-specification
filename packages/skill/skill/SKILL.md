...
- No delivery flag, no modules, no actors, no read-model element, no operations on a value
  ...
  laws; `references/model-reference.md` is exactly the model, and `references/preferences.md`
  lists every preference with what it costs, which is what to answer with when a user asks why
