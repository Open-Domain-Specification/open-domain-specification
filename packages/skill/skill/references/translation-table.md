...
| "we hold it until X and Y have both happened", "the checkout", "the onboarding", "order to delivery" | Process | `processes.<id>` with `starts`, `on`, `then`, `ends` | `bc.addProcess(name, {description}).starts(e1).on(e2).then(op).ends(e3)` |
