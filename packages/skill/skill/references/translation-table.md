...
| "the checksum has to add up", "both amounts are in one currency" | Invariant of the **value object**, holding by construction | `boundedcontexts.<id>.valueobjects.<id>.invariants.<id>`, `constrains` naming its own attributes | `vo.addInvariant(name, {description}).constrains(attribute)` |
...
| "the pickup date has to be before the delivery date", "the weight must be positive" (checked on the request, nothing stored yet) | Precondition constraining the request's fields | `invariants.<id>` with `precondition: true`, `constrains` naming the operation and attributes of its schema | `bc.addInvariant(name, {description, precondition: true}).constrains(operation, field)` |
| "approve it only if the customer is in good standing", "start it only with a valid entitlement" (checked against a fact fetched from another context) | Precondition constraining the answer that call came back with | `invariants.<id>` with `precondition: true`, `constrains` naming the transition, the operation of this context that makes the call, and an attribute of what that call returns | `agg.addInvariant(name, {description, precondition: true}).constrains(transition, check, answerField)` |
| "every itinerary we come back with meets the deadline they asked for", "no quote we return is outside the band" (a guarantee about the answer) | Postcondition constraining the answer's fields | `invariants.<id>` with `postcondition: true`, `constrains` naming the operation and attributes of what it returns or rejects with | `bc.addInvariant(name, {description, postcondition: true}).constrains(operation, field)` |
...
| "it returns a list of ...", "an array of matches", "all the ones that ..." | The same schema, marked many | operation `returns: {$ref, many: true}` | `provides(name, {type: "operation", returns: {of: s, many: true}})` |
...
| "opening a claim starts the whole thing off", "submitting the application kicks it off" (a command, not a fact, begins an instance) | Process started by its own context's operation | `processes.<id>` with `starts` naming that operation | `bc.addProcess(name, {description}).starts(openClaim).ends(e)` |
