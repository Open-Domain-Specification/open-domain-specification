...
describe("every template, through the shipped route", () => {
	...
	it("ConsumablePage: a query whose answer is a list says so in the fact and the section", () => {
		const container = draw(PETSTORE_REFS.manyQuery);
		expect(
			[...container.querySelectorAll("dt")].map((t) => t.textContent),
		).toContain("Returns many");
		const returns = container.querySelector("#returns");
		// The count of attributes sits in the heading beside the title.
		expect(returns?.querySelector("h2")?.textContent?.trim()).toContain(
			"Returns many",
		);
		// The shape is the one the list is of, so its attributes are drawn.
		expect(returns?.textContent).toContain("petId");
	});
	...
	it("SchemaPage: a schema that only ever comes back lists the operations that return it, one of them many", () => {
		...
		// PetSummary comes back one at a time from GetPetSummary and as a list
		// from the search, and the direction column says which.
		const directions = new Map(rows.map((r) => [r[0], r[2]]));
		expect([...directions.keys()].map((it) => it?.split("\n")[0])).toEqual([
			"FindPetsByStatus",
			"GetPetSummary",
		]);
