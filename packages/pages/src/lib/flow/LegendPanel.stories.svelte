<script module lang="ts">
import {
	ODSConsumableMap,
	ODSContextMap,
	ODSFlowMap,
	ODSRelationMap,
} from "@open-domain-specification/core";
import { defineMeta } from "@storybook/addon-svelte-csf";
import { petstoreModel } from "../fixtures";
import {
	consumableGraph,
	contextGraph,
	flowGraph,
	relationGraph,
} from "./graph";
import Harness from "./LegendPanel.harness.svelte";

const { workspace } = petstoreModel();
const sales = workspace.boundedcontexts.get("sales_bc")!;
const order = sales.aggregates.get("order")!;
const fulfilment = sales.processes.get("order_fulfilment")!;
const { Story } = defineMeta({
	title: "Flow/LegendPanel",
	component: Harness,
});
</script>

<Story name="Context map" args={{ graph: contextGraph(ODSContextMap.fromWorkspace(workspace)), kind: "context" }} />
<Story name="Consumable map" args={{ graph: consumableGraph(ODSConsumableMap.fromBoundedContext(sales)), kind: "consumable" }} />
<Story name="Relation map" args={{ graph: relationGraph(ODSRelationMap.fromAggregate(order)), kind: "relation" }} />
<Story name="Flow map" args={{ graph: flowGraph(ODSFlowMap.fromBoundedContext(sales), fulfilment.ref), kind: "flow" }} />
<!-- What a map with no room for the legend shows: the header row alone, ready to open. -->
<Story name="Collapsed by the fit" args={{ graph: contextGraph(ODSContextMap.fromWorkspace(workspace)), kind: "context", crowded: true }} />
