...
/**
 * A UML class for the relation map, in three compartments: the stereotype
 * line and the name, the attributes with {id} identity markers and types,
 * and an operations compartment the core has nothing to fill yet, kept so
 * the box reads as a class. The cluster path (the aggregate's context path)
 * sits below. Mirrors the Graphviz image; `floating` hides the fixed handles.
 */
...
<div class={`flow-card relation-node ${tone}`} title={data.id}>
	...
	<div class="head compartment">
		<span class="stereotype">{stereotype}</span>
		<strong>{data.label}</strong>
	</div>
	<ul class="attrs compartment">
	...
	<div class="operations compartment">&nbsp;</div>
...
<style>
	...
	.compartment + .compartment { border-top: 1px solid var(--fg); }
	.relation-node.muted .compartment + .compartment { border-top-style: dashed; }
	...
	.attrs { list-style: none; margin: 0; padding: 6px 12px; font-family: var(--mono); font-size: 11px; white-space: nowrap; text-align: left; }
	.operations { padding: 4px 12px; font-size: 11px; }
