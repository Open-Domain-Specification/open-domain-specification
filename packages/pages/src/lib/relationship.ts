 ...
 * templates and the extension's search all read it, and the extension's
 * bundle must not pull the Svelte Flow or Graphviz graph modules in behind it.
 */
 ...
 * strategic position table, the search spotlight. A relationship has no name
 * of its own, so it is named by its two contexts and the direction between
 * them: an arrow when one side leads, a double arrow when neither does.
 */
