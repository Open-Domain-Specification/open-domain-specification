...
Triggers have widened. `starts` may name an operation of the process's own context (third amendment of 2026-09-09, card 99). `on` and `ends`, and a policy's `on`, may name an answer by its origin: `<op>/returns`, `<op>/rejects/<schema>`, a rejection `reason` (decision 25, card 114) or `completed` (decision 13, card 99); naming an answer by schema alone no longer holds (second and third amendments of 2026-09-08, cards 92 and 94). An answer of an operation the reactor itself issues is a trigger without a consumption (2026-09-09, card 95). An answer routes to the caller only, by the `by` that made the call, the single-consumption inference applying where no `by` is written (fourth amendment of 2026-09-09, card 100; amendment of 2026-09-10, card 104). The second amendment's sentence that the answer is synchronous because the operation is no longer holds; see the fourth note of 2026-09-10 and decision 15 (card 120). A command to a live instance goes to the application service, which raises the fact the process hears (third note of 2026-09-10).

`reaction-cycle`'s lifecycle test: a ring is a process's own lifecycle when the process is its only reactor, whether or not it crosses a boundary (amendment of 2026-09-08, card 81; note of 2026-09-09, card 102's narrowing reverted); also when every other reactor is a policy hearing the process's event through an anti-corruption-layer consumption and raising its own context's events (amendment of 2026-09-10, card 108), but only where the process hears the ring's event in `on` or `ends`; a translated event in `starts` spawns instances and is reported (second note of 2026-09-10, card 113), as is any re-entry through `starts` (card 104). A second process entering by `starts` and leaving by `ends` counts as a call; two processes each re-entering while alive are still a cycle (second amendment of 2026-09-10, card 116).
