...
- [x] Design note in `docs/design/v2-specs/` (the designer writes, the design lead reviews if asked): node shapes for operation, event, policy and process; the dashed `ends` edge; how a process's lifecycle reads at a glance; where the diagram appears (context page, process page, policy page)
