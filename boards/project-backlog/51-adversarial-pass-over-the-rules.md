...
After cards 44 to 50 land, every rule in `packages/core/src/validate.ts` gets a hostile test: the smallest model that should trip it, the nearest model that must not, and the four reference models checked for diagnostics that fire on nothing real. the tester owns this; findings are cards or fixes, not opinions.
...
- [ ] Rules that overlap (a shape tripping two) are listed with a ruling request for the lead
