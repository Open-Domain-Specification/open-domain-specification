...
- Aggregates enforce invariants across entities and value objects (both carry typed attributes; an attribute may point at the value object that models its type, and identity attributes identify an entity); invariants list the entities, value objects or attributes they constrain; entity relations may carry a UML cardinality (1, 0..1, *, 1..*), and a reference across aggregates should target the other aggregate's root entity, and raise domain events (each with typed attributes). Commands on an aggregate capture what causes state change and list the events they raise. An aggregate publishes an event to other contexts through an event consumable that references it; an operation consumable may reference the command it exposes.
- Services provide operations that coordinate domain behavior.
- Policies (on a bounded context) react to domain events by issuing commands, possibly across contexts.
