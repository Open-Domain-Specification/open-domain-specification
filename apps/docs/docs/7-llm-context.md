...
- Aggregates enforce invariants across entities and value objects, and raise domain events (each with typed attributes). Commands on an aggregate capture what causes state change and list the events they raise. An aggregate publishes an event to other contexts through an event consumable that references it; an operation consumable may reference the command it exposes.
