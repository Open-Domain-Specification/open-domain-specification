...
- Aggregates enforce invariants across entities and value objects (both carry typed attributes; an attribute may point at the value object that models its type, and identity attributes identify an entity); invariants list the entities, value objects or attributes they constrain; entity relations may carry a UML cardinality (1, 0..1, *, 1..*), and a reference across aggregates should target the other aggregate's root entity. An aggregate or service provides consumables typed event or operation; an operation lists the events it raises, either may point at one of the context's schemas (payload shapes with typed attributes), and a consumable marked internal is never offered to other contexts.
- Services provide operations that coordinate domain behavior.
- Policies (on a bounded context) react to event consumables by issuing operation consumables, possibly across contexts.
- Each bounded context carries a glossary: the terms of its ubiquitous language, each with a definition, optional aliases, and optionally the element that embodies it.
