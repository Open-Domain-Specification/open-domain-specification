...
| `attribute-relation-coherence` | warning | an attribute typed by a value object has the matching `uses` relation, and the two agree about number: `*` or `1..*` for a list, `0..1` or `*` for an optional attribute, `1` or `1..*` for a required one. Where several relations point at one value object, the relation whose `label` is the attribute's name is the match. The type itself is free text and is never checked against the value object's name |
