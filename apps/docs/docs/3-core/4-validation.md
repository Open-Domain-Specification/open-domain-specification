...
| `consumption-agreement` | warning | a consumption crossing a pair that holds more than one directed relationship in that direction names in `relationship` which agreement the exchange belongs to, and what it names is an agreement between those two contexts in that direction. A pair with one agreement is asked nothing; a crossing reported here belongs to no agreement, so `relationship-roles-backed` says nothing about it until it names one |
...
| `mud-needs-acl` | warning | a consumption from a big ball of mud declares the `anti-corruption-layer` downstream role, and an identity naming one is taken in behind a layer too |
