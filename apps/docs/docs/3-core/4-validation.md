...
| `invariant-in-aggregate` | error | every element an invariant constrains is inside the invariant's own aggregate — an entity, an attribute or one of its consumables — or is a value object of its context, or one borrowed from another context that something in the aggregate holds, or is an operation of a service of its own context that guards it. Naming an operation says which operation keeps the rule, not what kind of rule it is; `precondition` and `postcondition` say that. A precondition may also constrain attributes of a schema its guarded operation takes, returns or rejects with, a postcondition those of what it returns or rejects with, and both follow composition into the shapes those compose |
| `invariant-in-context` | error | every element a context's invariant constrains belongs to that context: an entity or attribute of any of its aggregates, a value object something in the context holds, its own or a borrowed one, or one of its operations. A precondition or a postcondition reaches the payload shapes its guarded operation carries, composition included |
| `context-invariant-is-checked` | error | a context's invariant is a check, so it names at least one operation of that context that makes it — before that operation acts, or of what it answers with. Which side of the call the check falls on is said with `precondition` or `postcondition`, and neither claims the rule holds at rest; a flagged rule is asked for its guard by the two names-operation rules instead, so the model is told once |
...
| `relationship-declared` | warning | two contexts joined by a crossing — a consumption of the other's consumable, a policy or process reacting to the other's event or waiting on an answer of its operations, or an attribute typed by one of its value objects — declare a relationship, in either direction. An identity crossing is not one of them: the context map draws it under «id», which is its record, and a relationship is declared where something is exchanged or a language is borrowed. The arrow is the author's claim about who dictates the model, not about who calls whom, so it answers the question the crossing raises whichever way it points; separate ways answers nothing, because it contradicts the crossing |
...
| `relationship-cycle` | warning | the directed relationships whose traffic is calls form no cycle; a step carried only by events is choreography, and a call whose consumption declares an anti-corruption layer translates at its edge, so neither counts; the layer is read per consumption, so one translated call does not excuse the untranslated ones beside it (decision 20). The message lists the ring's contexts in order |
...
| `external-is-boundary` | error | an external context declares no aggregates, no policies, no processes, no invariants and no internal operations, and is not a big ball of mud as well: one is somebody else's system, the other the enterprise's own |
