...
| `context-invariant-guarded` | error | a context's invariant names at least one operation of that context as the operation that checks it |
| `context-invariant-is-checked` | error | a context's invariant is neither a precondition nor a postcondition: a rule across instances is always a check made before acting, so saying so adds nothing and promising it holds afterwards is a guarantee the boundary cannot keep |
