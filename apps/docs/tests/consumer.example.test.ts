...
describe("Consumable Example", () => {
	it("should compile ", async () => {
		const svg = await consumableMapToDigraph(
			ODSConsumableMap.fromWorkspace(ws),
		).toSVG();
		expect(svg).toMatchSnapshot();
		writeFileSync("./static/img/consumable-map-example.svg", svg);
	});
});
