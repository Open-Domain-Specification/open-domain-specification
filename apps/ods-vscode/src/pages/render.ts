...
${section(
	...
	(await figure(
		input.svg,
		"Context map",
		contextMapToDigraph(ODSContextMap.fromWorkspace(ws)).toDot(),
	)) +
		(contexts.length
...
${section(
	...
	await figure(
		input.svg,
		`${d.name} context map`,
		contextMapToDigraph(ODSContextMap.fromDomain(d)).toDot(),
	),
)}`;
	return {
		title: d.name,
...
${section(
	...
	await figure(
		input.svg,
		`${s.name} context map`,
		contextMapToDigraph(ODSContextMap.fromSubdomain(s)).toDot(),
	),
)}`;
	return {
		title: s.name,
...
${section(
	...
	(relationships.length
		...
		(await figure(
			input.svg,
			`${bc.name} context map`,
			contextMapToDigraph(ODSContextMap.fromBoundedContext(bc)).toDot(),
		)),
...
${section(
	...
	(await figure(
		...
		consumableMapToDigraph(ODSConsumableMap.fromBoundedContext(bc)).toDot(),
	)) +
...
${section(
	...
	await figure(
		input.svg,
		`${a.name} relation map`,
		relationMapToDigraph(ODSRelationMap.fromAggregate(a)).toDot(),
	),
...
${section(
	...
	(await figure(
		...
		consumableMapToDigraph(ODSConsumableMap.fromAggregate(a)).toDot(),
...
${section(
	...
	(await figure(
		...
		consumableMapToDigraph(ODSConsumableMap.fromService(s)).toDot(),
	)) +
